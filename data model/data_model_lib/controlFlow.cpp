...
{
    Map* controlBlockTable = (Map*)(get_attribute_ref_al("control_block_list"));
...
{
    universal_data* tmp = this->get_attribute_ref_al("label");
...
void initialPatternTable()
{
    ...
    declar = CLM_data_declaration("Parking_Place*", "Var");
    func = CLM_function_call("Employee.get_Link_Employee_Parking_Place", paraList);
    
    stmt = CLM_assignment_statement(declar, func);
    tmp.set_value("1");
    Map tmpList;
    tmpList.insert(tmp, stmt);
    
    pattern_table->add_attribute_al("NAVI(Emplyee,Assigned)", tmpList);
    
    declar = CLM_data_declaration("Product*", "Var");
    func = CLM_function_call("Product_Line.get_Head_Link_Product_Line_Product", paraList);
    
    stmt = CLM_assignment_statement(declar, func);
    tmp.set_value("1");
    Map tmpList2;
    tmpList2.insert(tmp, stmt);
    
    pattern_table->add_attribute_al("NAVI(Product_Line,Belong_To)", tmpList2);
    
    declar = CLM_data_declaration("Order*", "Var");
    func = CLM_function_call("Product.get_Head_Link_Product_Order", paraList);
    
    stmt = CLM_assignment_statement(declar, func);
    tmp.set_value("1");
    Map tmpList3;
    tmpList3.insert(tmp, stmt);
    
    declar = CLM_data_declaration("list<Customer*>", "Var");
    func = CLM_function_call("Order.get_List_Link_Order_Customer", paraList);
    
    stmt = CLM_assignment_statement(declar, func);
    tmp.set_value("2");
    tmpList3.insert(tmp, stmt);
    
    pattern_table->add_attribute_al("NAVI(Product,Order)", tmpList3);
}

Map* findPattern(string pName)
{
    return (Map*)(pattern_table->get_attribute_ref_al(pName));
}

void importControlModel(control_flow_model* cfm)
{
    string label;
    ifstream cmInput("./controlModel.txt");
    list<node*> cTree;
            ...
            while(pattern != "end_for_loop"){
                ...
                if(pattern == "body") {
                    ...
                    if(tmp == "pattern"){
                        ...
                        Map* tmp = findPattern(dataAccess);
                        for(tmp->begin();!tmp->end();(*tmp)++){
                            bodyList.push_back((node*)(tmp->value()));
                        }
                    }else{
                        lop = tmp;
                        cmInput >> op;
            ...
            while(pattern != "end_if_else_block"){
                ...
                if(pattern == "if_body") {
                    ...
                    if(tmp == "pattern"){
                        ...
                        Map* tmp = findPattern(dataAccess);
                        for(tmp->begin();!tmp->end();(*tmp)++){
                            ifBodyList.push_back((node*)(tmp->value()));
                        }
                    }else{
                        lop = tmp;
                        cmInput >> op;
                ...
                if(pattern == "else_body") {
                    ...
                    if(tmp == "pattern"){
                        ...
                        Map* tmp = findPattern(dataAccess);
                        for(tmp->begin();!tmp->end();(*tmp)++){
                            elseBodyList.push_back((node*)(tmp->value()));
                        }
                    }else{
                        lop = tmp;
                        cmInput >> op;
...
node* createParseTree(control_flow_model* cfm)
{
    ...
    Map* controlBlockTable = (Map*)(cfm->get_attribute_ref_al("control_block_list"));
    for(controlBlockTable->begin();!controlBlockTable->end();(*controlBlockTable)++){
        universal_data* udTmp = controlBlockTable->value();
    ...
    return root;
//    //for
//    node* init = CLM_assignment_statement("test", "1");
//    node* condition = CLM_binary_expression("test", "<", "10");
//    node* iteration = CLM_assignment_statement("test", "test+1");
//    node* body = CLM_assignment_statement("test2", "test2+test");
//    
//    node* forLoop = CLM_for_loop(init, condition, iteration, body);
//    
//    //if_else
//    node* ifCondtion = CLM_binary_expression("test", "<", "10");
//    node* ifBody = CLM_assignment_statement("test2", "test2+test");
//    node* elseBody = CLM_assignment_statement("test2", "test2+1");
//    
//    node* ifElse = CLM_if_else_statement(ifCondtion, ifBody, elseBody);
//    
//    //fuction call
//    list<string> paramList;
//    string functionName, param;
//    
//    functionName = "E1.setAttr1";
//    param = "test";
//    paramList.push_back(param);
//    
//    node* functionCall = CLM_function_call(functionName, paramList);
//    
//    //while
//    node* whileCond = CLM_binary_expression("test", "<", "10");
//    node* whileBody = CLM_assignment_statement("test2", "test2+test");
//    
//    node* whileLoop = CLM_while_loop(whileCond, whileBody);
//    
//    cTree.push_back(forLoop);
//    cTree.push_back(ifElse);
//    cTree.push_back(functionCall);
//    cTree.push_back(whileLoop);
//    
//    return CLM_C_code(cTree);
}
